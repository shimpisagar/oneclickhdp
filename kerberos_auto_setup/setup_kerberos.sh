...
configure_kerberos()
{
	...
	echo -e "\n `ts` Creating the KERBEROS_CLIENT host components for each host"
                ...
                for client in "${KERBEROS_CLIENTS[@]}"
                do
                        KRB5_AGENT=`echo $client`
                        curl -H "X-Requested-By:ambari" -u $AMBARI_ADMIN_USER:$AMBARI_ADMIN_PASSWORD -i -X POST -d '{"host_components" : [{"HostRoles" : {"component_name":"KERBEROS_CLIENT"}}]}' http://$AMBARI_HOST:8080/api/v1/clusters/$CLUSTER_NAME/hosts?Hosts/host_name=$KRB5_AGENT
                        sleep 1
                done
